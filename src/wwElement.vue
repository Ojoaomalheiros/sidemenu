<template>
  <nav ref="sidemenuRef" class="sidemenu" :style="computedStyle">
    ...
    <div class="sidemenu-nav">
      ...
      >
        <wwElement v-if="item.icon" :ww-props="item.icon" class="nav-item-icon"></wwElement>
        <span v-else class="nav-item-icon" v-html="defaultIcon"></span>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Default icon for items without icon selected
    ...
    const handleMenuItemClick = (item) => {
      ...
      if (item?.url && typeof wwLib !== 'undefined') {
        try {
          ...
          // If wwLib.goTo fails (page not found), try using window.location
